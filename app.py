...
def cart():
    ...
    if session['type'] == 'powner':
        ...
        dis = 0.00
        val = 1
    return render_template("cart.html", items=items, val=val, subtotal=subtotal, items_len=items_len, coupon=coupon,
                           dis=dis, buid=buid)
...
def coupon():
    ...
    if session['type'] == 'customer':
        if couponcode == 'SOMAIYA15':
            ...
            dis = 0.00
            val = 1
            return render_template("cart.html", items=items, val=val, subtotal=subtotal, items_len=items_len,
                                   coupon=coupon, dis=dis, buid=buid)
        else:
...
def mhome():
    ...
    buid = buyid()
    return render_template("home.html", categories=categories, categories1=categories1, categories2=categories2,
                           subtotal=subtotal, len_items=len_items, buid=buid)
